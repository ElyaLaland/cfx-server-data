...
local INVENTORY_RESOURCE = 'inventaire' -- nom de ton script d'inventaire dans server.cfg

local function GiveStarterItems(source)
    if GetResourceState(INVENTORY_RESOURCE) ~= 'started' then
        print('^3[qb-multicharacter]^7 Inventaire (' .. INVENTORY_RESOURCE .. ') non démarré, objets de départ ignorés.')
        return
    end
    ...
    for _, v in pairs(sharedStarterItems) do
        ...
        -- On protège l'appel : si l'inventaire n'a pas cet export (pas encore configuré),
        -- on ignore juste cet objet au lieu de faire planter la création du personnage.
        local ok, err = pcall(function()
            exports[INVENTORY_RESOURCE]:AddItem(src, v.item, v.amount, false, info, 'qb-multicharacter:GiveStarterItems')
        end)
        if not ok then
            print('^3[qb-multicharacter]^7 Impossible de donner l\'objet "' .. tostring(v.item) .. '" (export AddItem introuvable dans "' .. INVENTORY_RESOURCE .. '"). On continue sans bloquer.')
        end
    end
end
...
RegisterNetEvent('qb-multicharacter:server:loadUserData', function(cData)
    ...
    if QBCore.Player.Login(src, cData.citizenid) then
        ...
        loadHouseData(src)
        -- Reconnexion : on saute toujours le menu de spawn et on remet le joueur là où il était
...
RegisterNetEvent('qb-multicharacter:server:createCharacter', function(data)
    ...
    if QBCore.Player.Login(src, false, newData) then
        ...
        if GetResourceState('qb-apartments') == 'started' and Apartments.Starting then
            ...
            -- Premier personnage : on laisse le joueur choisir son point de spawn (Legion, Police, Paleto, Motel...)
            TriggerClientEvent('qb-spawn:client:setupSpawns', src, { citizenid = Player.PlayerData.citizenid }, false, nil)
